...
def train4():
    ...
    class LoggerHook(tf.train.SessionRunHook):
        ...
        def before_run(self, run_context):
            self._step += 1
            return tf.train.SessionRunArgs(model.loss)  # Asks for loss value.

        def after_run(self, run_context, run_values):
            if self._step % cfg.log_freq == 0:
                current_time = time.time()
                duration = current_time - self._start_time
                self._start_time = current_time

                loss_value = run_values.results
                examples_per_sec = cfg.log_freq * cfg.batch_size / duration
                sec_per_batch = float(duration / cfg.log_freq)
    ...
    with model.graph.as_default():
        ...
        hooks = []
        hooks.append(tf.train.StopAtStepHook(last_step=cfg.max_steps))
        hooks.append(tf.train.NanTensorHook(model.loss))
        if cfg.log_freq > 0:
            hooks.append(tf.train.StepCounterHook(output_dir=cfg.ckpt_dir, every_n_steps=cfg.log_freq))

        hooks.append(
            tf.train.SummarySaverHook(save_steps=cfg.log_freq, output_dir=cfg.ckpt_dir, summary_writer=summary_writer,
                                      summary_op=model.summary_op))
        hooks.append(tf.train.CheckpointSaverHook(save_steps=cfg.save_freq, checkpoint_dir=cfg.ckpt_dir, saver=saver))

    """Set Session settings."""
    config = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)
    config.gpu_options.allow_growth = True

    sess = tf.train.MonitoredTrainingSession(hooks=hooks, config=config)
    ...
    with sess:
        ...
        for epoch in range(cfg.epoch):
            ...
            """Save model at each step periodically"""
            ckpt_file = os.path.join(cfg.ckpt_dir, 'model_{:.4f}.ckpt'.format(loss_value))
            # saver.save(sess, ckpt_file, global_step=g_step)
